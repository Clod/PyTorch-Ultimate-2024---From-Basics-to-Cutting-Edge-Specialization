...
X_test, y_test = next(iter(test_loader))  # Changed this line
with torch.no_grad():
    y_pred = model(X_test) 
    y_pred = y_pred.round()
...
# loaded_model.classifier = nn.Sequential(OrderedDict([
#     ('fc1', nn.Linear(1024, 1)),
#     ('Output', nn.Sigmoid())
# ]))
# loaded_model.load_state_dict(torch.load('transfer_learning_model.pth'))
# loaded_model.eval()
