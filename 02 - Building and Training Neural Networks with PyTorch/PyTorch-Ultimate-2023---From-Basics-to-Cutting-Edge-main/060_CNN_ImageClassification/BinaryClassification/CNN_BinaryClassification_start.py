...
class ImageClassificationNet(nn.Module):
    def __init__(self) -> None:
        ...
        self.fc1 = nn.Linear(16 * 6 * 6, 128) # out: (BS, 128)
...
for epoch in range(NUM_EPOCHS):
    for i, data in enumerate(trainloader, 0):
        ...
        optimizer.zero_grad()
        # forward pass
        outputs = model.forward(inputs) # model(inputs)
        # calc losses
        # reshape(-1, 1) = Take the original tensor and reshape it into
        # a vector, so that it can be reshaped into a column vector, 
        # where the number of rows is inferred from the original tensor.
...
# %%
acc = accuracy_score(y_test, y_test_pred) # compare y_test and y_test_pred
